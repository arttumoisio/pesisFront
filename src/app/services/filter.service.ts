...
export class FilterService {
  ...
  private filters: Array<{string: string; operator: string; column: string; }> = [{
    string: '',
    operator: '',
    column: '',
  }];
  ...
  setIntFilters(filters: Array<{string: string; operator: string; column: string; }>): void {
    this.filters.splice(1);
  ...
  removeIntFilter(filter: {string: string; operator: string; column: string; }): void {
    const index: number = this.filters.findIndex((value) => {
      return value === filter;
    });
    if (index === 0 || index === -1) {
      return;
  ...
  getFilters(): Array<{string: string; operator: string; column: string; }> {
    return this.filters;
  }
  getStrFilters(): Array<{string: string; operator: string; column: string; }> {
    return this.filters.slice(0, 1);
  }
  getIntFilters(): Array<{string: string; operator: string; column: string; }> {
